...
from ai_escape_mrc.managed_agent.output_schema import CloudPayload

# WIKI-CONSULTED: function-replacement-convention#delete-in-same-commit
# WIKI-FINDING: replaces run_ai_escape_mrc.py + LangGraph FSM atomically in Phase C
# WIKI-ACTION: this file is the new entry point; legacy run_ai_escape_mrc.py deleted in same commit (Phase C)
...
def ensure_agent(client: anthropic.Anthropic, system_prompt: str) -> str:
    """Create (or reuse by name) the skill-ai-escape-mrc-v1 agent. Returns agent_id.
    ...
    """
    agent_name = "skill-ai-escape-mrc-v1"
    ...
    agent = client.beta.agents.create(
        ...
        description=(
            "AI Escape MRC analyst for ecosystem failure modes. "
...
def ensure_environment(client: anthropic.Anthropic) -> str:
    ...
    betas: list[anthropic.types.AnthropicBeta] = [ANTHROPIC_BETA]
    env_name = "skill-ai-escape-mrc-env"
...
def create_cloud_session(problem: str, agent_id: str, api_key: str) -> str:
    ...
    session = client.beta.sessions.create(
        ...
        title=f"AI Escape MRC run - {problem[:60]}",
        betas=betas,
    )
...
def _load_system_prompt() -> str:
    ...
    sp_path = Path(__file__).parent / "ai_escape_mrc" / "managed_agent" / "system_prompt.md"
...
def handle_session_complete(
    ...
    """Validate payload (R13 fail-closed), write artifacts, deliver final report.
    ...
    stage_summaries_path = run_dir / "stage-summaries.md"
    stage_summaries_jsonl_path = run_dir / "stage-summaries.jsonl"
    if validated.stage_summaries:
        stage_summaries_path.write_text(
            "\n\n".join(validated.stage_summaries) + "\n",
            encoding="utf-8",
        )
        with stage_summaries_jsonl_path.open("w", encoding="utf-8") as f:
            for index, summary in enumerate(validated.stage_summaries, start=1):
                json.dump(
                    {
                        "ts": datetime.now(timezone.utc).isoformat(),
                        "phase": f"managed_agent_stage_{index}",
                        "title": f"Managed Agent Stage {index}",
                        "screen": summary,
                        "markdown": summary,
    ...
    recipients = resolve_delivery_recipients(
        user_email=user_email or validated.user_email,
    ...
    delivery_doc = {
        ...
        "screen_summary": validated.stage_summaries[-1] if validated.stage_summaries else None,
        "stage_summaries_path": str(stage_summaries_path) if validated.stage_summaries else None,
    ...
    delivery = _send_completion_email(
        run_id=run_dir.name,
        run_dir=run_dir,
    ...
    delivery_doc["email_delivery_result"] = "sent" if delivery.get("ok") else "failed"
    delivery_doc["email_delivery_error"] = delivery.get("error")
    delivery_doc["email_delivery"] = delivery
    delivery_status_path = run_dir / "delivery-status.json"
    delivery_status_path.write_text(json.dumps(delivery_doc, ensure_ascii=False, indent=2), encoding="utf-8")
    return {
...
def _send_completion_email(
    ...
    """Send consolidated email with report path + plan preview.
    Per R13: all inline; no see-file pointers for required actions.
    """
    try:
        from ai_escape_mrc.delivery.email import send_markdown_email
    except ImportError:
        msg = "send_markdown_email unavailable"
        print(f"WARN: {msg}; report and plan artifacts were still written", file=sys.stderr)
        recipients = recipients or resolve_delivery_recipients()
        return _delivery_result(False, "import_error", recipients, error=msg)
    plan_inline = delivery_doc.get("plan_inline", "")
    plan_first50 = "\n".join(plan_inline.splitlines()[:50])
    plan_truncated = "..." if len(plan_inline.splitlines()) > 50 else ""
    recipients = recipients or resolve_delivery_recipients()
    body = (
        f"# AI Escape MRC Run {run_id} -- Report Ready\n\n"
        f"**Recipient:** `{recipients.display()}`\n\n"
        f"**Report path:** {delivery_doc.get('report_path', '(none)')}\n\n"
        f"**Plan path:** {delivery_doc.get('plan_path', '(none)')}\n\n"
        f"**Stage summaries:** {delivery_doc.get('stage_summaries_path') or '(none)'}\n\n"
        f"## Latest Stage Summary\n{delivery_doc.get('screen_summary') or '(no stage summary)'}\n\n"
...
def _send_error_email(
    ...
    try:
        ...
        return False
    return send_markdown_email(
        subject=f"[AI ESCAPE MRC ERROR] {run_id}: {error_subject}",
        body_md=error_body,
        recipients=recipients,
    )
...
def _normalize_delivery_result(value, recipients: DeliveryRecipients) -> dict:
...
def _cmd_run(problem: str, user_email: str | None = None, operator_email: str | None = None) -> int:
    ...
    client = _get_client(api_key)
    betas: list[anthropic.types.AnthropicBeta] = [ANTHROPIC_BETA]
    recipients = resolve_delivery_recipients(user_email=user_email, operator_email=operator_email)

    try:
        session_id = create_cloud_session(problem, agent_id="skill-ai-escape-mrc-v1", api_key=api_key)
    except Exception as e:
        err_body = f"# Session Creation Failed\n\nRun ID: {run_id}\nError: {type(e).__name__}: {e}\n"
        _send_error_email(run_id, "session creation failed", err_body, recipients=recipients)
        print(f"ERROR: session creation failed: {e}", file=sys.stderr)
        return 1

    print(f"[trigger_ai_escape_mrc] session created: {session_id}; run_id: {run_id}")
    _post_telegram_heartbeat(f"`{datetime.now().isoformat(timespec='seconds')}` AI Escape MRC `{run_id[:20]}` session_created")
    ...
    while True:
        ...
        try:
            ...
            print(f"[trigger_ai_escape_mrc] poll error: {e}; retrying next interval", file=sys.stderr)
        ...
        try:
            ...
            for ev in events_page:
                ...
                _post_telegram_heartbeat(f"`{datetime.now().isoformat(timespec='seconds')}` AI Escape MRC `{run_id[:16]}` {ev_type}")
        except Exception:
            pass
        ...
        if status in ("idle", "terminated"):
            ...
            if not payload:
                err_body = (
                    f"# AI Escape MRC Cloud Session Produced No Parseable Payload\n\n"
                    f"Run ID: {run_id}\nSession: {session_id}\nStatus: {status}\n\n"
                    f"The session ended but no JSON payload was found in agent message events."
                )
                _send_error_email(run_id, "no payload found", err_body, recipients=recipients)
            ...
            try:
                ...
                print(f"[trigger_ai_escape_mrc] final delivery complete; run_id: {run_id}")
                _post_telegram_heartbeat(f"`{datetime.now().isoformat(timespec='seconds')}` AI Escape MRC `{run_id[:20]}` completed")
                return 0
            except ValidationError as e:
                err_body = (
                    f"# AI Escape MRC Payload Schema Validation Failed\n\n"
                    f"Run ID: {run_id}\nSession: {session_id}\n\n"
                    f"## Validation Errors\n```\n{e}\n```\n\n"
                    f"No artifacts written (R13 fail-closed)."
                )
                _send_error_email(run_id, "payload schema invalid", err_body, recipients=recipients)
                print(f"ERROR: payload schema validation failed:\n{e}", file=sys.stderr)
                return 2

        if status in ("failed", "errored"):
            err_body = f"# AI Escape MRC Cloud Session Failed\n\nRun ID: {run_id}\nSession: {session_id}\nStatus: {status}\n"
            _send_error_email(run_id, f"session status={status}", err_body, recipients=recipients)
            print(f"ERROR: session {session_id} ended with status={status}", file=sys.stderr)
            return 2

        print(f"[trigger_ai_escape_mrc] session {session_id} status={status}; waiting...", file=sys.stderr)


def _cmd_status(run_id: str) -> int:
    delivery = Path(__file__).parent / "runs" / run_id / "delivery-status.json"
    if not delivery.exists():
        print(f"No delivery status for {run_id}")
        return 0
...
def _cmd_resume_approval(run_id: str, approve: bool, reject_reason) -> int:
    print(
        ...
        "AI Escape MRC now ends at final report/plan delivery.",
        file=sys.stderr,
    )
    return 2
...
def main() -> int:
    ap = argparse.ArgumentParser(prog="trigger_ai_escape_mrc")
    ...
    if args.problem:
        return _cmd_run(args.problem, user_email=args.user_email, operator_email=args.operator_email)
