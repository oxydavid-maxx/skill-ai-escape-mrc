...
from ai_escape_mrc.validators import validate_phase9_plan, PLAN_MIN_BYTES
...
def test_phase9_empty_llm_output_raises_contract_error(tmp_path):
    ...
    with patch("ai_escape_mrc.phases.phase_9_write_plan.call_claude", return_value=""):
        with pytest.raises(Phase9OutputContractError) as exc_info:
...
def test_phase9_too_short_llm_output_raises_contract_error(tmp_path):
    ...
    with patch("ai_escape_mrc.phases.phase_9_write_plan.call_claude",
               return_value="# Plan\n\nToo short."):
        with pytest.raises(Phase9OutputContractError) as exc_info:
            phase_9_write_plan(state)
...
def test_phase9_valid_llm_output_passes(tmp_path):
    ...
    with patch("ai_escape_mrc.phases.phase_9_write_plan.call_claude", return_value=valid_plan):
