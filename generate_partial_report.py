...
markdown AI Escape MRC report. Skips phases 6 and 7 (never reached due to Phase 5
...
def render_report(state: dict, events: list[dict]) -> str:
    ...
    lines = []
    lines.append(f"# AI Escape MRC Report (PARTIAL ??pipeline crashed at Phase 5 run 2): {problem[:80]}")
    lines.append("")
    lines.append(f"**Date**: {date}")
    lines.append(f"**Problem**: {problem}")
    lines.append(f"**Run ID**: {run_id}")
    lines.append(f"**Total elapsed**: {total_sec/60:.1f} min")
    lines.append("**Model**: environment default (via claude_agent_sdk / active CLI session)")
    lines.append("")
    ...
    lines.append("|---|---|---|")
    q1 = str(why_chains.get("q1_trc_nc", {}).get("root", "(missing)"))[:150]
    q2 = str(why_chains.get("q2_trc_nd", {}).get("root", "(missing)"))[:150]
    q3 = str(why_chains.get("q3_mrc_nc", {}).get("root", "(missing)"))[:150]
    q4 = str(why_chains.get("q4_mrc_nd", {}).get("root", "(missing)"))[:150]
    ...
    lines.append("| | NC | ND |")
    lines.append("|---|---|---|")
    c1 = str(corrective.get("q1_trc_nc", {}).get("action", "(missing)") if isinstance(corrective.get("q1_trc_nc"), dict) else corrective.get("q1_trc_nc", "(missing)"))[:150]
    c2 = str(corrective.get("q2_trc_nd", {}).get("action", "(missing)") if isinstance(corrective.get("q2_trc_nd"), dict) else corrective.get("q2_trc_nd", "(missing)"))[:150]
    c3 = str(corrective.get("q3_mrc_nc", {}).get("action", "(missing)") if isinstance(corrective.get("q3_mrc_nc"), dict) else corrective.get("q3_mrc_nc", "(missing)"))[:150]
    c4 = str(corrective.get("q4_mrc_nd", {}).get("action", "(missing)") if isinstance(corrective.get("q4_mrc_nd"), dict) else corrective.get("q4_mrc_nd", "(missing)"))[:150]
    ...
    def p_action(q):
        ...
        if isinstance(pq, dict):
            return str(pq.get("action", "(missing)"))[:150]
        return str(pq)[:150]
    ...
    for q_key in ("q1_trc_nc", "q2_trc_nd", "q3_mrc_nc", "q4_mrc_nd"):
        ...
        lines.append("")
        lines.append(f"**Root**: {chain.get('root', '(missing)')}")
        lines.append("")
...
def _resend_existing(report_path: Path) -> None:
    """Read an existing report from disk and re-deliver via send_ai_escape_mrc_report_email.
...
def main():
    ...
    default_reports_dir = SKILL_ROOT / "docs" / "ai-escape-mrc-reports"
    out_path = Path(os.environ.get("CLAUDE_AI_ESCAPE_MRC_REPORTS_DIR", str(default_reports_dir))) / f"ai-escape-mrc-{date_str}-{slug}-partial.md"
    ...
    # Send email
    try:
        log = send_ai_escape_mrc_report_email(report_md, out_path, state.get("problem", "")[:200])
        print(f"Email: {log}")
    except Exception as e:
        print(f"Email FAIL: {e}")
