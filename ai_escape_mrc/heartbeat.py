"""Heartbeat thread for long-running AI Escape MRC runs.
...
Started automatically by run_ai_escape_mrc.py at launch. Runs as a daemon thread so it exits
...
# WIKI-ACTION: built-in (daemon thread inside run_ai_escape_mrc.py), not external - removes the
